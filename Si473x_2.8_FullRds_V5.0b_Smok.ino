//  V5.0b    02-03-2022 -  MW , LW, SW indicator for MW,LW and SW Bands in AM mode - 200Khz step in FM
//  V5.0a    03-02-2022 -  S-Meter Led style- Full RDS Services - BFO Bug solved
...
const char *Rel_Date[]   =    { "02-03-2022"};
const char *Rel_Vers[]   =    { "5.0b Beta"};
...
int Xsmtr   =   0;
int Ysmtr   =  85 + ytotoffset;  // S meter 85
...
Step sp[] = {
  { 1 , 20   ,Xfstep, 200, Yfstep, 50,  0},
  { 5 , 10   ,Xfstep, 200, Yfstep, 50, 50},
  { 9 ,  1   ,Xfstep, 200, Yfstep, 50, 100},
  { 10,  0   ,Xfstep, 200, Yfstep, 50, 150}
};
//======================================================= End Tunings Steps     ===============================
...
Bandwidth bw[] = {                                 //  AM    SSB   FM
  { 4 , 4 , 0 , XfBW, 200, YfBW, 30,   0},         //  1.0   0.5   Aut
  { 5 , 5 , 1 , XfBW, 200, YfBW, 30,  30},         //  1.8   1.0   110
  { 3 , 0 , 2 , XfBW, 200, YfBW, 30,  60},         //  2.0   1.2    84
  { 6 , 1 , 3 , XfBW, 200, YfBW, 30,  90},         //  2.5   2.2    60
  { 2 , 2 , 4 , XfBW, 200, YfBW, 30, 120},         //  3.0   3.0    40
  { 1 , 3 , 0 , XfBW, 200, YfBW, 30, 150},         //  4.0   4.0   ---
  { 0 , 0 , 0 , XfBW, 200, YfBW, 30, 180}          //  6.0   ---   ---
};
...
Band band[] = {
  ...
  {   "FM", FM_BAND_TYPE,  FM,  6400, 10800,  9890, 10, 0, 0}, //  FM          0
  {   "LW", LW_BAND_TYPE,  AM,   100,   514,   198,  9, 0, 0}, //  LW          1
  {   "MW", MW_BAND_TYPE,  AM,   514,  1800,  1008,  9, 0, 0}, //  MW          2
  {"BEACON", LW_BAND_TYPE,  AM,   280,   470,   284,  1, 0, 0}, // Ham          3
  ...
  {  "80M", SW_BAND_TYPE, LSB,  3500,  3900,  3600,  1, 0, 0}, // Ham   80M    8
  ...
  {  "19M", SW_BAND_TYPE,  AM, 14500, 17000, 15700,  5, 0, 0}, //       19M   19
  {  "17M", SW_BAND_TYPE, USB, 18000, 18300, 18100,  1, 0, 0}, // Ham   17M   20
  {  "16M", SW_BAND_TYPE,  AM, 17000, 18000, 17600,  5, 0, 0}, //       16M   21
  {  "15M", SW_BAND_TYPE,  AM, 18000, 21000, 18950,  5, 0, 0}, //       15M   22
  {  "15M", SW_BAND_TYPE, USB, 21000, 21500, 21074,  1, 0, 0}, // Ham   15M   23
  {  "13M", SW_BAND_TYPE,  AM, 21450, 24850, 21500,  5, 0, 0}, //       13M   24
  {  "12M", SW_BAND_TYPE, USB, 24850, 25500, 24940,  1, 0, 0}, // Ham   12M   25
  {  "11M", SW_BAND_TYPE,  AM, 25500, 26100, 25800,  5, 0, 0}, //       11M   26
  {   "CB", SW_BAND_TYPE,  AM, 26000, 28000, 27125,  1, 0, 0}, // CB band     27
  {  "10M", SW_BAND_TYPE, USB, 28000, 30000, 28500,  1, 0, 0}, // Ham   10M   28
  {  "SW",  SW_BAND_TYPE,  AM,   100, 30000, 14000,  5, 0, 0}  // Whole SW    29
#endif
...
const int lastButton = (sizeof bt / sizeof(Bigbutton)) - 1;
...
void setup() {
  ...
  Serial.println("     SI4735 DSP Radio");
...
//=======================================================================================
  ...
  if (currentMode != FM) {
    ...
    if ((rssi > 64) and (rssi <= 74)) spoint = 170 + (rssi - 64) * 2;  // S9 +30  //156
    if ((rssi > 74) and (rssi <= 84)) spoint = 182 + (rssi - 74) * 2;  // S9 +40  //172
    if ((rssi > 84) and (rssi <= 94)) spoint = 200 + (rssi - 84) * 2;  // S9 +50  //188 
    if  (rssi > 94)                   spoint = 204;                    // 204 per S9 +60
    if  (rssi > 95)                   spoint = 208;                    // 208 per >S9 +60
  }
  else
  {
    ...
    if ((rssi > 44) and (rssi <= 54)) spoint = 170 + (rssi - 44) * 2;  // S9 +30  //156
    if ((rssi > 54) and (rssi <= 64)) spoint = 182 + (rssi - 54) * 2;  // S9 +40  //172
    if ((rssi > 64) and (rssi <= 74)) spoint = 200 + (rssi - 64) * 2;  // S9 +50  //188 
    if  (rssi > 74)                   spoint = 204;                    // 204 per S9 +60
    if  (rssi > 76)                   spoint = 208;                    // 208 per >S9 +60
  }

   tft.fillRect(Xsmtr + 15, Ysmtr + 43 , (2 + spoint), 4, TFT_RED); // riga rossa per i led dello smeter (da 0 a 4 )
   
  if (spoint > 45) { 
    tft.fillRect(Xsmtr + 63, Ysmtr + 41 , (2 + spoint - 40), 6, TFT_ORANGE); // riga arancione per i led dello smeter ( da 4 a 9)
  }
  if (spoint > 105) { 
    tft.fillRect(Xsmtr + 125, Ysmtr + 38 , (2 + spoint - 60), 9, TFT_GREEN); // riga verde per i led dello smeter ( da 9 a 9+60)
  }

for (int i = 0; i < 19; i++) // numero righe trasversali nere per i led smeter
tft.fillRect(Xsmtr + 15 + (i * 12), Ysmtr + 38, 3, 10, TFT_BLACK); // traccia righe trasversali nere per i led smeter spaziate di 12 pixel
  tft.fillRect(Xsmtr + 17 + spoint, Ysmtr + 38 , 226 - (2 + spoint), 9, TFT_BLACK);  // pulisce il fondo nero dello smeter
}
...
//=======================================================================================
  ...
  tft.fillRect(XVolInd + 15, YVolInd + 14 , (2 + vol), 6, TFT_RED);
  tft.fillRect(XVolInd + 17 + vol, YVolInd + 14 , 215 - (2 + vol), 6, TFT_BLACK);
}
...
//=======================================================================================
  ...
  tft.fillRect(XVolInd + 15, YVolInd + 14 , (2 + vol), 6, TFT_RED);
  tft.fillRect(XVolInd + 17 + vol, YVolInd + 14 , 215 - (2 + vol), 6, TFT_LIGHTGREY);
}
...
//=======================================================================================
            ...
            if (n == 11) {// SET button
              ...
              if(Displayfreq < 1) {
                ...
                if ((Displayfreq > 30) and (Displayfreq < 64.0 )) {
                  tft.setCursor(7, 25);
                  tft.print("Freqency > 30Mhz and < 64.0 MHz");
                  ErrorBeep();
                }else{
                  if ((Displayfreq >= 108) and (Displayfreq < 153 )) {
                    ...
                    if ((Displayfreq >= 64.0) and (Displayfreq <= 108)) {
  ...
  if (currentPRES != previousPRES)
  {
    ...
    DrawDispl();
    tft.fillRect(XFreqDispl + 0, YFreqDispl + 19 , 228, 48, TFT_BLACK);// lBack freq. field   +6 
    AGCfreqdisp();
    tft.setTextColor(TFT_YELLOW, TFT_BLACK );
    tft.setTextSize(2);
...
//=======================================================================================
  if (( currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
    if ( currentFrequency != previousFrequency ) {
      previousFrequency = currentFrequency;
      tft.fillRect(XFreqDispl + 60, YFreqDispl + 57, 130, 20, TFT_BLACK);  // clear RDS stationName
      tft.fillRect(0, YFreqDispl + 78, 240, 17, TFT_BLACK);// clear RDS rdsMsg
      tft.fillRect(0, YFreqDispl + 57, 80, 20, TFT_BLACK);// clear RDS rdsTime
    }
    if ((RDS) and  (NewSNR >= 9)) checkRDS();  // 12
    else  tft.fillRect(XFreqDispl + 60, YFreqDispl + 57, 130, 20, TFT_BLACK); // clear RDS text stationName
  //        tft.fillRect(0, YFreqDispl + 78, 240, 17, TFT_BLACK);  // clear RDS rdsMsg
  }
}
...
//=======================================================================================
  ...
  if ((  currentMode == FM ) and ((FirstLayer) or (ThirdLayer))) {
    ...
    tft.fillRect(XFreqDispl + 196, YFreqDispl + 60 , 38, 11, TFT_RED); // STEREO MONO
    tft.drawString(buffer, XFreqDispl + 215, YFreqDispl + 70); 

    tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
    tft.setTextSize(3);
    tft.fillRect(0, 0 , 30, 25, TFT_BLACK);
    tft.drawString(" FM", XFreqDispl + 10, YFreqDispl + 28);
    }
   ...
   if ((band[bandIdx].bandType == MW_BAND_TYPE) and ((FirstLayer) or (ThirdLayer))and ( currentMode == AM )) {
    ...
    tft.setTextSize(3);
    tft.fillRect(0, 0 , 30, 25, TFT_BLACK);
    tft.drawString(" MW", XFreqDispl + 10, YFreqDispl + 28);
}
  if ((band[bandIdx].bandType == LW_BAND_TYPE) and ((FirstLayer) or (ThirdLayer))and ( currentMode == AM )){
     tft.setTextColor(TFT_MAGENTA, TFT_BLACK);
    tft.setTextSize(3);
    tft.fillRect(0, 0 , 30, 25, TFT_BLACK);
    tft.drawString(" LW", XFreqDispl + 10, YFreqDispl + 28); 
 ...
 if ((band[bandIdx].bandType == SW_BAND_TYPE) and ((FirstLayer) or (ThirdLayer))and ( currentMode == AM )){
    ...
    tft.setTextSize(3);
    tft.fillRect(0, 0 , 30, 25, TFT_BLACK);
    tft.drawString(" SW", XFreqDispl + 10, YFreqDispl + 28); 
 ...
 //   tft.setTextDatum(TL_DATUM);
  // tft.drawString("RSSI = " + String(NewRSSI) + " dBuV" , XFreqDispl + 8, YFreqDispl + 82);
 //   tft.drawString("DSP Radio Receiver" , XFreqDispl + 11, YFreqDispl + 82);
 //  tft.setTextDatum(TR_DATUM);
 //   tft.setTextColor(TFT_GREEN, TFT_BLACK);
 //   tft.drawString(" SNR= " + String(NewSNR) + " dB", XFreqDispl + 230, YFreqDispl + 82);
  }
  VOLbutoff();
}
...
//=======================================================================================
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setCursor(XVolInd +  11, YVolInd + 3);
  tft.print("0%.");
  tft.setCursor(XVolInd +  28, YVolInd + 3);
  tft.print("Vol...");
  tft.setCursor(XVolInd +  62, YVolInd + 3);
  tft.print("25%......");
  tft.setCursor(XVolInd + 116, YVolInd + 3);
  tft.print("50%......");
  tft.setCursor(XVolInd + 163, YVolInd + 3);
  tft.print("75%.....");
  tft.setCursor(XVolInd + 210, YVolInd + 3);
  tft.print("100%");
}
...
//=======================================================================================
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(XFreqDispl + 75, YFreqDispl + 62);
...
//=======================================================================================
  if ((FirstLayer) or (ThirdLayer)) {
    tft.setCursor(0, YFreqDispl + 79); //sulle coord di DSP radio...
    tft.print(rdsMsg);
  }
  delay(250);
}
...
if ((FirstLayer) or (ThirdLayer)) {
    ...
    tft.setCursor(0, YFreqDispl + 63); //sulle coord di rdsstation
    tft.print(rdsTime);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
      tft.setTextDatum(BC_DATUM);
    tft.setCursor(0, YFreqDispl + 51);
      tft.print(" Time");
  }
  delay(250);
}
  ...
  if ((FirstLayer) or (ThirdLayer)) {
    ...
    if ((VOLbut) or (AGCgainbut)) {
      ...
      if (band[bandIdx].bandType == FM_BAND_TYPE) {
        ...
        tft.setTextColor(TFT_WHITE, COLOR_BACKGROUND);
        tft.setFreeFont(&DSEG7_Classic_Mini_Regular_34);
        tft.drawString(String(Displayfreq, 2), XFreqDispl + 120, YFreqDispl + 58); //55
        tft.setTextSize(2);
        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        tft.setFreeFont(NULL); 
      ...
      if (currentMode == LSB || currentMode == USB  || currentMode == CW ) {
        if (bfoOn) {
         ...
         // tft.setFreeFont(&DSEG7_Classic_Mini_Regular_34);
          #ifdef IhaveCrystal
            tft.drawString(String(currentBFOmanu), XFreqDispl + 220, YFreqDispl + 61);
            tft.setTextSize(3);
            tft.setTextColor(TFT_YELLOW, COLOR_BACKGROUND);
          tft.setTextDatum(BR_DATUM);
                
          tft.drawString(String(Displayfreq/10, 0), XFreqDispl + 132, YFreqDispl + 61);  // /100 XD..+125
          tft.setFreeFont(NULL);
          tft.setTextSize(1);
          tft.drawString("'",XFreqDispl + 98, YFreqDispl + 70);   //   XFreqDispl+108
          ...
          #ifdef IhaveSI5351
          if (stepsizesynth == 10)  tft.fillRect(XFreqDispl + 170, YFreqDispl + 62, 20, 3, TFT_ORANGE);
          if (stepsizesynth ==  1)  tft.fillRect(XFreqDispl + 198, YFreqDispl + 62, 20, 3, TFT_ORANGE);
          #endif
          tft.setTextDatum(BC_DATUM);   
        }
        else {
          ...
          if (freqstepnr == 0)  tft.fillRect(XFreqDispl + 140, YFreqDispl + 62, 20, 3, TFT_ORANGE);
          if (freqstepnr == 1)  tft.fillRect(XFreqDispl + 170, YFreqDispl + 62, 20, 3, TFT_ORANGE);
          if (freqstepnr == 2)  tft.fillRect(XFreqDispl + 198, YFreqDispl + 62, 20, 3, TFT_ORANGE);
          tft.setTextDatum(BC_DATUM);
          tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        }
      }
...
//=======================================================================================
 if (FirstLayer or ThirdLayer) {
  ...
  //if (band[bandIdx].bandType != FM_BAND_TYPE) {
    ...
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    tft.drawString("Filter", XFreqDispl + 120, YFreqDispl + 8);
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    if (currentMode == FM) {
