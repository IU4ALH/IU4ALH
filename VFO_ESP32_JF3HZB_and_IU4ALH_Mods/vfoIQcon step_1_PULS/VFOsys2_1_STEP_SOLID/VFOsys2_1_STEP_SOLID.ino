...
void loop(void)
{
  ...
  {
  cnt = cnt-1; 
  if(cnt<0) { cnt=6 ; }  // cnt=6 se si usano i due pulsanti S+ e S-.... cnt=0 se si una un solo pulsante Step +
  Serial.println(cnt);
  delay(350); // 300
  }
  ...
  // Step control     
  if(count!=0){                           
  afstp=(int32_t)(0 + (count)*(freq_step ) );     //   afstp=(int32_t)(0.5 + (float)(count)*( (float)freq_step ) );
